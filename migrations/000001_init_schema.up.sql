...
CREATE TABLE worker_coffee_shop (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  worker_id uuid REFERENCES user(id),
  coffee_shop_id uuid REFERENCES coffee_shop(id),
  is_deleted boolean DEFAULT false,
  created_at timestamp DEFAULT CURRENT_TIMESTAMP
);
...
-- USER: поиск по телефону (авторизация)
CREATE INDEX idx_user_phone ON user(phone);
CREATE INDEX idx_user_role_id ON user(role_id);

-- IDEA: основные запросы (лента идей, фильтрация)
...
-- Составной индекс для фильтрации идей по кофейне + статус/категория
...
-- LIKE (или idea_like): подсчет голосов
CREATE INDEX idx_like_idea_id ON idea_like(idea_id);
CREATE INDEX idx_like_user_idea ON idea_like(user_id, idea_id); -- предотвращает дубли

-- REWARD: личный кабинет пользователя
